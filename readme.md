...
<h2>Software</h2>
To turn movement, and the variables from the environment, into sound, we build a deep learning model which processed 4 input parameters (left and right flex sensors, moisture sensor and wifi signal strength) and 3 output parameters controlling the sound engine written in Max/MSP.  The signals were combined into a string and passed into the ml5 multilayer regression model with Tensorflow architecture over UDP.  We chose the regression model vs classification because we wanted the sound to be continuous (e.g., a slide on the violin) rather than discrete (e.g., a key on the piano). The software was developed using p5js, ml5js, and various packages from p5js such as p5serial and p5sound libraries.  
